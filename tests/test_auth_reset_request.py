...
def test_reset_request_sends_email_for_signed_out_user(
    app: Flask, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Ensure signed-out users can request a reset email.
    ...
    Returns:
        None. Assertions verify email sends with an anonymous user context.
    ...
    user = User(email="signedout@example.com", role="customer")
    user.set_password("StrongPassw0rd!")
    db.session.add(user)
    db.session.commit()
