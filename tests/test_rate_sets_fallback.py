...
def _seed_zip_zone(
    zipcode: str, rate_set: str, zone: str = "Z1"
) -> ZipZone:
    row = ZipZone(zipcode=zipcode, zone=zone, rate_set=rate_set)
    db.session.add(row)
    db.session.commit()
    return row
...
def test_returns_row_for_requested_rate_set(app: Flask) -> None:
    _seed_zip_zone("85705", rate_set="science_care", zone="ARIZ")
...
def test_does_not_cycle_back_when_rate_set_is_already_default(
    ...
    _seed_zip_zone("85705", rate_set="science_care", zone="SCIE")
...
def test_multiple_kwargs_filter(app: Flask) -> None:
    _seed_zip_zone("85705", rate_set="science_care", zone="A")
    _seed_zip_zone("85706", rate_set="science_care", zone="B")
