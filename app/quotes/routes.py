...
def _normalize_and_validate_quote_id(quote_id_input: str | None) -> str | None:
    ...
    Args:
        quote_id_input: Raw value from ``request.values.get("quote_id")``.

    Returns:
        The uppercase, trimmed quote ID when input matches
        ``^Q-[A-HJ-NP-Z2-9]{8}$``; otherwise ``None``.

    External dependencies:
        Uses module-level :data:`READABLE_QUOTE_ID_PATTERN` to enforce the
        same alphabet generated by :func:`app.models.generate_readable_id`.
    """

    normalized_quote_id = (quote_id_input or "").strip().upper()
...
def lookup_quote():
    ...
    Inputs:
        Reads lookup fields from ``request.values`` so both ``GET`` query
        parameters and ``POST`` form submissions can resolve a quote.
        Validates ``quote_id`` as a readable ID generated by
        :func:`app.models.generate_readable_id`.
    ...
    if lookup_mode == "client_reference":
        normalized_client_reference = _normalize_and_validate_client_reference(
            request.values.get("client_reference")
        )
...
def _format_quote_copy_email_body(
    ...
    *,
    ...
    External dependencies:
        * Consumes quote pricing metadata generated by
    ...
    mileage = metadata.get("miles")
    mileage_text = (
        f"{float(mileage):,.2f}" if isinstance(mileage, (int, float)) else "N/A"
    )
    actual_weight = float(quote.actual_weight or 0.0)
    dim_weight = float(quote.dim_weight or 0.0)
    billable_weight = float(quote.weight or 0.0)
    uses_actual_weight = (quote.weight_method or "").strip().lower() == "actual"
    uses_dimensional_weight = (
        quote.weight_method or ""
    ).strip().lower() == "dimensional"
    ...
    lines.extend(
        [
            ...
            f"Actual Weight: {actual_weight:.2f} lbs"
            + ("  <-- WEIGHT USED FOR QUOTE" if uses_actual_weight else ""),
            f"Dimensional Weight: {dim_weight:.2f} lbs"
            + ("  <-- WEIGHT USED FOR QUOTE" if uses_dimensional_weight else ""),
            f"Weight Used for Quote: {quote.weight_method}",
            f"Billable Weight: {billable_weight:.2f} lbs",
            "",
            "PRICING BREAKDOWN",
            "------------------------------------------",
    ...
    for name, raw_cost in accessorials.items():
        ...
        lines.append(f"{safe_name}: $ {cost:.2f}")

    lines.extend(
        [
            f"Accessorials Total: $ {accessorial_total:.2f}",
            "",
            "Summary",
            "------------------------------------------",
            f"Quote Total: $ {float(quote.total or 0.0):.2f}",
...
def _format_quote_copy_email_html(
    ...
    *,
    ...
    base_charge = max(float(quote.total or 0.0) - accessorial_total, 0.0)
    mileage = metadata.get("miles")
    mileage_value = float(mileage) if isinstance(mileage, (int, float)) else None
    uses_actual_weight = (quote.weight_method or "").strip().lower() == "actual"
    uses_dimensional_weight = (
        quote.weight_method or ""
    ).strip().lower() == "dimensional"
    ...
    return render_template(
        ...
        show_mileage=(quote.quote_type or "").strip().lower() == "hotshot",
        uses_actual_weight=uses_actual_weight,
        uses_dimensional_weight=uses_dimensional_weight,
